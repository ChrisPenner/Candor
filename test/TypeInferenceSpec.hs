...
testUnify ::
     Monotype -> Monotype -> Either InferenceError (Substitutions, Monotype)
testUnify a b =
  runInference mempty $ do
    t <- (unify a b)
    subs <- use subMap
    return (subs, t)
...
spec = do
  describe "inferAppl" $ do
    it "returns proper application type vars" $ do
      runInference
        ...
        (inferAppl (Fix $ Builtin "+") [Fix $ Symbol "a", Fix $ Symbol "b"] *>
         use subMap) `shouldBe`
        Right [("a", intT), ("b", intT)]
    ...
    describe "unifies monotypes" $ do
      ...
      describe "TVars" $ do
        ...
        it "unifies type vars to anything with a substitution" $ do
          testUnify (TVar "a") (TVar "b") `shouldBe`
      ...
      describe "TFunc" $ do
        it "unifies simple functions" $ do
          testUnify (TFunc stringT intT) (TFunc stringT intT) `shouldBe`
            Right ([], TFunc stringT intT)
        it "unifies functions with required substitutions" $ do
          runInference
            mempty
            (unify (TFunc (TVar "a") (TVar "a")) (TFunc stringT intT)) `shouldBe`
  ...
  describe "infer" $ do
    ...
    it "Builtins" $ do
      testInference "++" `shouldBe`
        Right ([], TFunc stringT (TFunc stringT stringT))
    describe "Bindings" $ do
      it "infers binding types" $ do
        testInference "(= a 1)" `shouldBe` Right ([], TRows [("a", intT)])
    ...
    describe "FuncDefs" $ do
      it "infers type of functions" $ do
        testInference "{[a b] (+ a b)}" `shouldBe`
          Right ([("a", intT), ("b", intT)], (TFunc intT (TFunc intT intT)))
  ...
  describe "complex programs" $ do
    it "infers proper types with closures" $ do
      testInference "((= func ((= num 1) {[arg] (+ arg num)})) (func 10))" `shouldBe`
        Right ([("a", intT)], intT)
