...
spec = do
  describe "Parser" $ do
    ...
    it "parses funcs" $ do
      parse "(def [num] (* num 2))" `shouldBe` Right (Appl (Symbol "def") [List [Symbol "num"], (Appl (Symbol "*") [Symbol "num", Number 2])])
    it "parses definitions alongside expressions"
      (parse "((= times_two (def [num] (* num 2))) (times_two 5))" `shouldBe`
          Right (Appl  (Appl (Symbol "=") [Symbol "times_two", Appl (Symbol "def") [List [Symbol "num"], (Appl (Symbol "*") [Symbol "num", Number 2])]]) [Appl (Symbol "times_two") [Number 5]]))
    it "parses bools" $ do
