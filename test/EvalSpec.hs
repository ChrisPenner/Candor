...
spec = do
  describe "Eval" $ do
    ...
    it "evals nested bindings" $ do
      ((parse "((= times_two { [num] (* num 2) }) ((= ten (times_two 5)) (+ 5 ten)))" >>= eval) `shouldBe` Right (Number 15))
    ...
    it "can run simple recursive functions" $ do
      (parse "((= fact { [num] (if (== num 0) 1 (* num (fact (- num 1)))) }) (fact 0))" >>= eval) `shouldBe` Right (Number 1)
      parse "((= func { [cond] (if cond 5 (func F)) }) (func T))" `shouldBe` Right expectedRecursion
